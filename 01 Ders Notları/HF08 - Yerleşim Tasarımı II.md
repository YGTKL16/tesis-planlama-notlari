...
hafta: 8
durum: hazır
tags: [ders/tesis-planlama, algoritma/craft, algoritma/ikili-değişim, algoritma/grafik-tabanlı]
---
...
> [!tip] İki yöntemi birlikte kullan
...
> [!example] Ham slayttan basit uzaklık
...
> [!example] Kaynak örneği (slayt 36-41)
...
> [!warning] Ağırlık merkezi tuzağı: alan × koordinat
...
> Slayt 83 ikinci iterasyon için **3.310** yazar, ancak öğrenme paketinde slayt 73'teki iki matrisin bağımsız çarpım-toplamı `3A,2B,1C,5D,4E` ataması için **3.510** vermektedir. Sınavda kaynak değeri (3.310) beklenebilir; ancak matrisi elle çarparken 3.510 çıkarsa şaşırma — hesabını matris çarpımıyla **bağımsız doğrula**. Ayrıntı için → [[09 Öğrenme Paketleri/HF08C - CRAFT Yöntemi|HF08C]].

> [!info] Sınav stratejisi — 3.310 vs 3.510
> Sınavda iki matris sana verilirse: kendi çarpımını yap → **3.510** çıkarsa onu yaz, yanına parantez içinde `(Kaynak slayt: 3.310, hata)` not düş. Hem kendi hesabını hem kaynağı gören bir hoca puan kesmez; yalnız 3.310 yazar ve "neden?" sorusu gelirse açıklayamazsın.

> [!tip] Akılda kalıcı — CRAFT "Komşu takas oyunu"
> CRAFT şöyle çalışır: iki bölümü yer değiştir, toplam taşıma maliyeti ($\sum f_{ij} c_{ij} d_{ij}$) azaldı mı? Azaldıysa onayla, artmışsa geri al. Bunu tüm komşu çiftler için yap, en çok düşüreni uygula, değişme durana kadar tekrarla.
>
> **Tek cümle:** *"Hangi iki komşu dairemi değiştirsem tüm mahalle en az yol yürür?"*
>
> **Kritik tuzak:** Ağırlık merkezi **alan ağırlıklı** hesaplanır — geometrik merkez değil. Farklı alanlı bölümlerde $\bar{x} = \tfrac{\sum A_k x_k}{\sum A_k}$ kullan, slayttaki koordinatı kör kopyalama.
...
> [!warning] Komşuluk gerekli ama yeterli değil
...
> [!warning] Genel değişim uygunluğu kontrolü
...
- [[09 Öğrenme Paketleri/HF08A - İkili Yer Değişim Yöntemi|HF08A — İkili Yer Değişim Yöntemi]] (tam iterasyon, $\binom{n}{2}$, isim-konum tuzağı)
- [[09 Öğrenme Paketleri/HF08B - Grafik Tabanlı Yerleşim|HF08B — Grafik Tabanlı Yerleşim]] (düğüm-kenar-yüz, düzlemsellik, yüz puanı)
- [[09 Öğrenme Paketleri/HF08C - CRAFT Yöntemi|HF08C — CRAFT Yöntemi]] (üç matris, ağırlık merkezi, ΔC, slayt sayısal uyarıları)
