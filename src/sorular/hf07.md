...
??? success "Adım adım çözüm"
    ...
    **c)**
    ...
    $\Delta(\sum Fd) = +100+25-250-200-15+150 = -190$.
    Sonuç: $\Delta Z = 2\times(-190) = -380$ → $Z_{A\leftrightarrow D} = $ **2.680 TL** (tasarruf 380 TL, %12,4).
    Neden: A–E ve B–D yüksek akışları kısaldığından kazanç, A–B, A–C ve D–E kayıplarını aşar.

    **ç)**
    Ne yapıyoruz: B↔D takasını test ediyoruz.
    Formül/Yöntem: B ve D konumlarını değiştir, etkilenen çiftleri topla.
    Hesap: B(0,5)→(5,0) ve D(5,0)→(0,5) konumlarına geçer. Yalnız B veya D içeren çiftleri tara:
    B–C: $d$ 5→15 (+100), C–D: $d$ 15→5 (−30); diğer tüm çiftler sabit.
    $\Delta(\sum Fd) = +100-30 = +70$; $\Delta Z = 2\times70 = +140$.
    Sonuç: $Z_{B\leftrightarrow D} = 3.200$ TL → **kötüleşir**; A↔D çok daha iyidir.
    Neden: B–C yükü uzaklaşıp büyük katkı verir; C–D tasarrufu bunu telafi edemez.
